...
const GlobalExchange = ({setModal, userDetails}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const globalExchangesData = await getGlobalExchanges(userDetails.id);
                
                if (!globalExchangesData.exchanges || globalExchangesData.exchanges.length === 0) {
                    setExchangeResults([]);
                    setTotal(0);
                    setLoading(false);
                    return;
                ...
                for (const userExchange of globalExchangesData.exchanges) {
                    // Create user object
                    const user = {
    ...
    const getTotalStats = () => {
        ...
        exchangeResults.forEach(([user, exchanges]) => {
            exchanges.forEach(exchange => {
                ...
                totalSets.add(exchange.set.id);
            });
        });
        
        return { totalCanGet, totalCanGive, totalSets: totalSets.size };
    };
    ...
    const filteredResults = exchangeResults.filter(([user, exchanges]) => {
        ...
        return (
            ...
            exchanges.some(e => e.set?.name?.toLowerCase().includes(query))
        );
    });
    ...
    return (
        <div className="global-exchange-modal">
            <div className='modal-header modal-header--fixed'>
                <div className="modal-header__title">
                    <Icon name='exchange' color="#fff" width={24} height={24} />
            ...
            <div className='modal-content global-exchange-content'>
                <ConditionalRender if={loading}>
                    <div className="loading-container">
                        ...
                        <p className="loading-text">Finding exchange opportunities...</p>
                    </div>
                </ConditionalRender>
                ...
                <ConditionalRender if={!loading && total > 0}>
                    ...
                    <div className="exchange-stats">
                        <div className="stat-card stat-card--users">
                            <div className="stat-icon">
                                <Icon name='users' color="#6366f1" width={24} height={24} />
                            </div>
                        ...
                        <div className="stat-card stat-card--receive">
                            <div className="stat-icon">
                                <Icon name='download' color="#10b981" width={24} height={24} />
                            </div>
                            <div className="stat-info">
                                <span className="stat-value">{stats.totalCanGet}</span>
                                <span className="stat-label">Can Receive</span>
                            </div>
                        </div>
                        <div className="stat-card stat-card--give">
                            <div className="stat-icon">
                                <Icon name='upload' color="#f59e0b" width={24} height={24} />
                            </div>
                            <div className="stat-info">
                        ...
                        <div className="stat-card stat-card--sets">
                            <div className="stat-icon">
                                <Icon name='collections' color="#ec4899" width={24} height={24} />
                            </div>
                            <div className="stat-info">
                    ...
                    <div className="exchange-search">
                        <Icon name='search' color="#9ca3af" width={18} height={18} />
                        <input 
                            type="text"
                            placeholder="Search users or sets..."
                            value={searchQuery}
                                    ...
                                    >
                                        ...
                                        <div className="user-card__info">
                                            ...
                                            <div className="user-card__meta">
                                                ...
                                                <span className="exchange-badge exchange-badge--sets">
                                                    {exchanges.length} set{exchanges.length !== 1 ? 's' : ''}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="user-card__content">
                                            {exchanges.map((exchange, exchangeIndex) => (
                                                <div key={exchangeIndex} className="set-exchange-card">
                                                    <div className="set-exchange-card__header">
                                                        <Icon name='collections' color="#6366f1" width={16} height={16} />
                                                        <span>{exchange.set?.name}</span>
                                                    </div>
                ...
                <ConditionalRender if={showNoData}>
                    <div className="no-data-container">
                        <div className="no-data-icon">
                            <Icon name='exchange' color="#6b7280" width={64} height={64} />
                        </div>
                        <h3>No Exchanges Found</h3>
                        <p>
                            {users.length === 0
                                ? 'No users have sets in their collection yet.'
                                : 'No potential exchanges found with other users.'}
                        </p>
                    </div>
                </ConditionalRender>
            </div>
